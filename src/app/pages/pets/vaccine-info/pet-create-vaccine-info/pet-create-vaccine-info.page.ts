...
export class PetCreateVaccineInfoPage implements OnInit {
  ...
  async add(form: NgForm) {
    ...
    const isValidDate_ = isValidDate(this.vaccine.startDate);

      const isFutureDate_ = isFutureDate(this.vaccine.startDate);

      const isValidDate__ = isValidDate(this.vaccine.nextDate);

      if (!isValidDate_) {
        this.toastMessage.presentToast(
          'Por favor, agrega una fecha de aplicación válida'
        );
        return;
      } else if (isFutureDate_) {
        this.toastMessage.presentToast('Fecha de aplicación mayor a la actual');
        return;
      } else if (!isValidDate__) {
        this.toastMessage.presentToast(
          'Por favor, agrega una fecha próxima de aplicación válida'
        );
        return;
      }
