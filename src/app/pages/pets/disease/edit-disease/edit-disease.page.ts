...
export class EditDiseasePage implements OnInit {
  ...
  async update(form: NgForm) {
    if (form.valid) {
      ...
      const isValidDate_ = isValidDate(this.disease.diagnosticdate);

      const isFutureDate_ = isFutureDate(this.disease.diagnosticdate);

      if (!isValidDate_) {
        this.toastMessage.presentToast('Por favor, agrega una fecha válida');
        return;
      } else if (isFutureDate_) {
        this.toastMessage.presentToast('Fecha mayor a la actual');
        return;
      }
  ...
  async get() {
    this.diseaseService
      ...
      .subscribe(
        (response) => {
          response.diagnosticdate = this.formatDate(
            new Date(response.diagnosticdate)
          );
