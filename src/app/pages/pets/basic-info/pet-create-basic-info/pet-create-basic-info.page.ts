...
export class PetCreateBasicInfoPage implements OnInit, OnDestroy {
  ...
  async savePet(form: NgForm) {
    ...
    formData.append('castrated', this.pet.castrated);
    ...
    const isValidDate_ = isValidDate(this.pet.fechanacimiento)

    const isFutureDate_ = isFutureDate(this.pet.fechanacimiento)
    ...
    try {
      ...
      } else if (!isValidDate_){
        this.toastMessage.presentToast('Por favor, agrega una fecha de nacimiento válida');
      }else if (isFutureDate_){
        this.toastMessage.presentToast('Fecha de nacimiento mayor a la actual');
      }
       else {
        console.log('Form data',  this.user.session_token);
