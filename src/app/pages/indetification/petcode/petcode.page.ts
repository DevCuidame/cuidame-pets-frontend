...
export class PetcodePage implements OnInit {
  ...
  goToPets() {
    if (!this.showOptions && !this.showPersonaOptions) {
      this.navCtrl.navigateForward('/private/pages/home');
    } else {
      this.showOptions = false;
      this.showPersonaOptions = false;
    }
  ...
  getPetAgreements() {
    this.userService.getPetAgreements().subscribe(
      (data: any) => {
        this.petAgreements = data;
      },
      (error) => console.error('Error al obtener los acuerdos de mascotas:', error)
    );
  }
